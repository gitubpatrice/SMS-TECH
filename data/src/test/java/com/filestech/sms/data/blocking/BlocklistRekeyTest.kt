...
class BlocklistRekeyTest {
    ...
    fun `une entree heritee en forme internationale est convertie`() {
        ...
        val plan = planBlocklistRekey(listOf(entry(1, "+33612345678", "+33612345678")))

        assertThat(plan).containsExactly(RekeyAction.Update(1, "612345678"))
    }
    ...
    fun `une entree alphanumerique heritee sur cle vide est reconstituee depuis le libelle brut`() {
        ...
        val plan = planBlocklistRekey(listOf(entry(1, "SFR", "")))
    ...
    fun `deux ecritures d'un meme numero fusionnent en gardant la plus ancienne`() {
        val plan = planBlocklistRekey(
            listOf(
                entry(2, "0612345678", "0612345678", createdAt = 200),
                entry(1, "+33612345678", "+33612345678", createdAt = 100),
            ),
        )

        assertThat(plan).containsExactly(
            RekeyAction.Update(1, "612345678"),
            RekeyAction.Collapse(2, "612345678"),
        ).inOrder()
    }
    ...
    fun `une entree deja correcte n'entraine aucune ecriture`() {
        val plan = planBlocklistRekey(
            listOf(
                entry(1, "0612345678", "612345678"),
                entry(2, "SFR", "sfr"),
            ),
        )

        assertThat(plan).containsExactly(RekeyAction.Retain("612345678"), RekeyAction.Retain("sfr"))
    }
    ...
    fun `une entree dont la cle recalculee serait vide est laissee intacte`() {
        ...
        // ne casse rien.
        val plan = planBlocklistRekey(listOf(entry(1, "   ", "")))
    ...
    fun `rejouer la conversion sur une base deja migree ne produit que des conservations`() {
        ...
        val migrated = listOf(
            entry(1, "+33612345678", "612345678"),
            entry(2, "SFR 123", "sfr 123"),
            entry(3, "123", "123"),
        )

        val plan = planBlocklistRekey(migrated)
